...
class Jobs extends Component {
  ...
  changeEmployeeList = type => {
    ...
    const filteredEmployeeTypeList = employeeType.filter(
      eachEmployee => eachEmployee === type,
    )
    // console.log(filteredEmployeeTypeList)
    if (filteredEmployeeTypeList.length === 0) {
      this.setState({employeeType: [...employeeType, type]}, this.getJobs)
    } else {
      const removedTypeInEmploymentList = employeeType.filter(
        eachEmployee => eachEmployee !== type,
      )
      this.setState({employeeType: removedTypeInEmploymentList}, this.getJobs)
    }
  }
